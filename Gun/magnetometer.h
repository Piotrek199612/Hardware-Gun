...
class Magnetometer
{
  public:
  	static void init();
    static void process_magnetometer();
    static Vector readNormalize();
    static Vector readRaw();
    
  private:

    static float readDegrees();
   
  	static void setOffset(int xo, int yo,int zo);
  
  	static void setRange(hmc5883l_range_t range);
  
  	static void  setMeasurementMode(hmc5883l_mode_t mode);
  
  	static void  setDataRate(hmc5883l_dataRate_t dataRate);
  
  	static void  setSamples(hmc5883l_samples_t samples);
  
  	static float mgPerDigit;
  	static Vector v;
  	static  int xOffset, yOffset, zOffset;
  
  	static void writeRegister8(uint8_t reg, uint8_t value);
  	static uint8_t readRegister8(uint8_t reg);
  	static uint8_t fastRegister8(uint8_t reg);
  	static int16_t readRegister16(uint8_t reg);
};
